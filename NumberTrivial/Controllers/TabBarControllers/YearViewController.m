...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (![_numberField.text isEqualToString:@""]) {
        ...
        if ([dict valueForKey:@"date"]) {
            NSString *dateString = [NSString stringWithFormat:@"%@ ,%@", [dict valueForKey:@"date"], [dict valueForKey:@"number"]];
            _numberField.text = dateString;
        }else{
            NSString *dateString = [NSString stringWithFormat:@"%@", [dict valueForKey:@"number"]];
            _numberField.text = dateString;
        }
...
- (IBAction)pushRandomButton:(id)sender {
    ...
    NSDictionary *jsonDictionary = [_api getDictionaryWithParameter:@"" withCategory:@"year" isRandom:YES];
    _resultTextView.text = [jsonDictionary valueForKey:@"text"];
    if ([jsonDictionary valueForKey:@"date"]) {
        NSString *dateString = [NSString stringWithFormat:@"%@ ,%@", [jsonDictionary valueForKey:@"date"], [jsonDictionary valueForKey:@"number"]];
        _numberField.text = dateString;
    }else{
        NSString *dateString = [NSString stringWithFormat:@"%@", [jsonDictionary valueForKey:@"number"]];
        _numberField.text = dateString;
    }
    if ([[UIScreen mainScreen] bounds].size.height < 568){
        [_resultTextView setFont:[UIFont fontWithName:_resultTextView.font.fontName size:20]];
